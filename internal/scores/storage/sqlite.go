...
func (s *Sqlite) DeleteByJudgeRole(cardId, boutId uint, judgeRole string) error {
	return s.db.Where("card_id = ? AND bout_number = ? AND judge_role = ?", cardId, boutId, judgeRole).Delete(&Score{}).Error
}
